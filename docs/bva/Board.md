...
| TC30 | Board with injected harbor whose `vertex1Id` maps to no vertex | Harbor still appears in `getHarbors()`; no vertex is assigned that harbor on the vertex1 side (BVA: `v1 == null` branch in `createHarbors()`) | :x: |
| TC31 | Board with injected harbor whose `vertex2Id` maps to no vertex | Harbor still appears in `getHarbors()`; no vertex is assigned that harbor on the vertex2 side (BVA: `v2 == null` branch in `createHarbors()`) | :x: |
